...
Sync 有两个实现（子类），分别为 NonfairSync（非公平锁）和 FairSync（公平锁），默认是非公平锁，我们看 FairSync 部分。
...
1. 首先会尝试获取锁tryAcquire（该方法由子类自己实现，AQS不提供实现），如果此时当前锁没有线程在持有且阻塞队列为空，则CAS操作获取锁，如果当前锁已经被持有，则判断持有的线程是不是当前线程，如果是则重入（ReentrantLock的情况）
2. 如果tryAcquire失败，则将当前线程封装成节点放入到阻塞队列中，通过CAS操作设置为阻塞队列的队尾（如果队列为空则需要经历两次，一次初始化队头，另一次设置尾节点）
3. 将当前节点设置到阻塞队列中后，则调用AQS的acquireQueued方法，如果当前节点是阻塞队列的头节点则尝试再次CAS获取锁（获取锁成功后会将阻塞队列的对头设置为自己）。
4. 3、4是一个死循环，如果3抢锁失败的话则判断阻塞队列的状态，如果前一个节点的状态值为-1，则当前线程直接挂起，如果不是则往前找状态为-1的节点，这样前一个节点释放锁后才能唤醒当前节点（新创建节点状态值是0，会被后继节点设为-1，大于0的话表示取消了排队）。
5. 处理完队列后当前线程就挂起了。
...
static final class FairSync extends Sync {
    ...
    /**
       该方法交由子类重写里面的逻辑，AQS在该方法中是直接抛出异常
    ...
    protected final boolean tryAcquire(int acquires) {
        ...
        // state是父类的私有属性，父类的私有属性子类只能拥有不能使用，因此通过父类的保护方法getState使用
        int c = getState();
        // state == 0 此时此刻没有线程持有锁
        if (c == 0) {
            ...
            if (!hasQueuedPredecessors() &&
                ...
                // 到这里就是获取到锁了，标记一下，告诉大家，现在是我占用了锁，该方法也是AQS的
                setExclusiveOwnerThread(current);
                return true;
            }
        }
        // 如果锁已经使用了，则判断当前拥有该锁的线程是不是当前线程，如果是，则重入
    ...
    // 参数mode此时是Node.EXCLUSIVE，代表独占模式（EXCLUSIVE是一个null的静态Node对象）
    private Node addWaiter(Node mode) {
        ...
        **/
        ...
        // 仔细看看上面的代码，如果会到这里，
        // 说明 pred==null(队列是空的) 或者 CAS失败(有线程在竞争入)
        enq(node);
        return node;
    }
  
    // 采用自旋的方式入队
    // 之前说过，到这个方法只有两种可能：等待队列为空，或者有线程竞争入队（有竞争就一直用CAS获取，直到成功添加到队尾），
    ...
    private Node enq(final Node node) {
        for (;;) {
            ...
            if (t == null) { // Must initialize
                // 初始化head节点，注意只是初始化head节点，没有将等待的线程设置到队列中
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                ...
                // 到这里，说明上面的if分支没有成功（不是阻塞队列队头（注意不是指head）或者CAS失败）
    ...
    // 当前节点不是阻塞队列对头或者CAS抢锁失败就会进入到这个方法。
...
public final boolean release(int arg) {
    ...
    if (tryRelease(arg)) {
        ...
        if (h != null && h.waitStatus != 0)
            // 唤醒后继节点，注意阻塞队列中有节点获取到锁，就会把head节点替换成自己（公平锁的情况下）
            unparkSuccessor(h);
        return true;
    }
    return false;
...
3. 阻塞队列。因为争抢锁的线程可能很多，但是只能有一个线程拿到锁，其他的线程都必须等待，这个时候就需要一个 queue 来管理这些线程，AQS 用的是一个 FIFO 的队列，就是一个链表，每个 node 都持有后继节点的引用。AQS 采用了 CLH 锁的变体来实现，感兴趣的读者可以参考这篇文章[关于CLH的介绍]( http://coderbee.net/index.php/concurrent/20131115/577)（CLH是人名，自旋锁），写得简单明了。
