...
CountDownLatch自己总结的运行过程：

1. CountDownLatch是一个共享式的锁，当有线程调用await()方法的时候会进入到阻塞队列中，直到其他线程调用coutnDown()方法将state值减1（每次只能减1），减state值减为0的最后一个线程会唤醒阻塞队列中等待的线程。
2. 首先第一个被唤醒的会将前一个节点的状态值由-1变为0，然后自己成为head节点，再唤醒下一个节点。

CountDownLatch （倒计时锁）这个类是比较典型的 AQS 的共享模式的使用，这是一个高频使用的类。latch 的中文意思是**门栓、栅栏**，具体怎么解释我就不废话了，大家随意，看两个例子就知道在哪里用、怎么用了。
...
有了 CountDownLatch 的基础后，分析 Semaphore （信号量）会简单很多。Semaphore 是什么呢？它类似一个资源池（读者可以类比线程池），每个线程需要调用 acquire() 方法获取资源，然后才能执行，执行完后，需要 release 资源，让给其他的线程用。
